 ...
 * Statically cached via Next.js unstable_cache with tag-based invalidation.
 */
export const getCollectionAnalytics = unstable_cache(
  async (timeframe: AnalyticsTimeframe = "90d"): Promise<AnalyticsData> => {
    ...
    const [
      ...
      // Category Copy Counts (lightweight)
    ...
    for (const book of booksByCategoryWithCopies) {
    ...
    // 6. Top Borrowed Books Leaderboard (Optimized - fetch only top 6 titles)
