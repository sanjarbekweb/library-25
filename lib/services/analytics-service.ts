...
import { subDays, subMonths, subYears, format, isBefore } from "date-fns";
...
/**
 * Core business domain fetcher for library aggregate analytics.
 */
async function fetchRawCollectionAnalytics(timeframe: AnalyticsTimeframe = "90d"): Promise<AnalyticsData> {
  const now = new Date();
  const startDate = getStartDateForTimeframe(timeframe);

  // 1. Fetch overview counts and aggregations in parallel
  const [
    totalBooks,
    totalCopies,
    availableCopies,
    ...
    // Loans in timeframe (accelerated by @@index([borrowedAt]))
    ...
    // Category Copy Counts (lightweight selection)
  ...
  for (const book of booksWithCopyCounts) {
    ...
    existing.copyCount += book._count.copies;
  ...
  // 6. Top Borrowed Books Leaderboard (Fetch only top 6 titles)
  ...
  const topBookIds = Array.from(bookLoanCountMap.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, 6)
    .map(([bId]) => bId);

  const topBooksData = topBookIds.length > 0
  ...
  const topBooks: TopBorrowedBook[] = topBooksData
    .map((book) => {
  ...
  // 7. Active Reader Cohorts (Top Borrowers - fetch only top 5 users)
 ...
 * Statically cached via Next.js unstable_cache with timeframe parameterization & tag-based invalidation.
 */
export const getCollectionAnalytics = unstable_cache(
  async (timeframe: AnalyticsTimeframe = "90d"): Promise<AnalyticsData> => {
    return fetchRawCollectionAnalytics(timeframe);
  },
  ["collection-analytics-data"],
  { revalidate: CACHE_TTL.MEDIUM, tags: [CACHE_TAGS.ANALYTICS] }
);
